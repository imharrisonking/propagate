...
export default function CircularText({ text, fontSize, textColour }) {
	...
	return (
		<Link href="/demo" className="flex items-center align-middle">
			<div
				className="flex justify-center items-center relative h-[88dvh] w-[88dvh]"
			...
			>
				...
				>
					...
					{characters.map((char, index) => {
						...
						const x =
							radius + textRadius * Math.cos((angle - rotation) * (Math.PI / 180));
						const y =
							radius + textRadius * Math.sin((angle - rotation) * (Math.PI / 180));
